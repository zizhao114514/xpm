...
XPM_INSTALLED = f"{XPM_ROOT}/installed"
XPM_LOG = f"{XPM_ROOT}/log"

def ensure_dirs():
    for d in [XPM_ROOT, XPM_DB, XPM_CACHE, XPM_INSTALLED, XPM_LOG]:
...
def load_status():
    ...
    try:
        ...
        return {"installed": {}, "version": "2.1-0"}
...
def main():
    ...
    if cmd == "verify":
        ...
        if r.returncode == 0:
            ...
            # 记录到 status
            db = load_status()
            pkg_name = os.path.basename(deb).split("_")[0]
            if "installed" not in db:
                db["installed"] = {}
        ...
        if not args:
            print("用法: xm remove <包名>")
            return 1
        for pkg in args:
            r = subprocess.run(["dpkg", "-r", pkg], capture_output=True, text=True)
            if r.returncode == 0:
                print(f"[✓] 已卸载: {pkg}")
                db = load_status()
                if pkg in db.get("installed", {}):
                    del db["installed"][pkg]
                    save_status(db)
            else:
                print(f"[!] {pkg}: {r.stderr.strip()[:80]}")
    else:
        print(f"未知命令: {cmd}")
